...
class lifeCache
{
    ...
    function __destruct()
    {
    	if (!$caches = self::read('caches-lifetypes'))
    		$caches = array();
    	...
    	$this->write('caches-lifetypes', $caches, 3600 * 24 * 7 * 4 * 12);
    }
    ...
    {
        if (!class_exists('lifeCache' . ucfirst($name))) {
    ...
    function engine($name = 'file', $settings = array())
    {
        ...
        $cacheClass = 'lifeCache' . ucfirst($name);
        $_this =& lifeCache::getInstance();
    ...
    {
    	if ($key!='caches-lifetypes') {
    		if (!$caches = self::read('caches-lifetypes'))
    			$caches = array();
    		$caches[$key] = time().$duration;
    		self::write('caches-lifetypes', $caches, 3600 * 24 * 7 * 4 * 12);
    	}
    	if ($key!='caches-lifetypes') {
